...
def get_link(keyWord, pageRange):
    ...
    for page in range(int(pageRange)):
        ...
        crawlingUrl = URL_BEFORE_KEYWORD + keyWord + URL_BEFORE_PAGE_NUM + str(currentPage)

        response = requests.get(crawlingUrl)
...
def full_vis_bar(by_num):
    ...
    matplotlib.re("font", family=font_name, size=10)
    plt.title("그래프 제목")
    plt.xlabel("단어")
    plt.ylabel("개수")
...
def wordcloud(file1):
    with open(file1, "r", encoding="utf-8") as f:
        ...
        wc = WordCloud(font_path="Apple", background_color=(168,237,244),
...
def main(argv):
    if len(argv) != 4:
        print("잘못된 사용법 입니다.")
        print("사용법: python [파일이름] [키워드] [가져올 페이지 숫자] [TopN개수 입력]")
        return
